...
def start_paper_trading():
    ...
    logger.info("=" * 60)
    logger.info("📈 TEST 1: Paper Trading 시작")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("✅ TEST 1 완료 (3회 iteration 완료)\n")

    except Exception as e:
        logger.error(f"✗ Paper Trading 실패: {e}", exc_info=True)
        notifier.notify_error(f"[테스트] Paper Trading 실패: {e}", context="TEST 1")
        current_trader = None


def stop_paper_trading():
    """
    TEST 2: Paper Trading 종료 및 리포트 생성 (2분 후 실행)
    """
    global current_trader

    logger.info("=" * 60)
    logger.info("📊 TEST 2: Paper Trading 종료 및 리포트 생성")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("✅ TEST 2 완료\n")
        current_trader = None

    except Exception as e:
        logger.error(f"✗ 트레이딩 세션 중지 실패: {e}", exc_info=True)
        notifier.notify_error(f"[테스트] 트레이딩 세션 중지 실패: {e}", context="TEST 2")
...
def main():
    ...
    스케줄:
    - 즉시: Paper Trading 시작 (3회 iteration)
    ...
    logger.info("테스트 스케줄:")
    logger.info("  즉시 - Paper Trading 시작 (3회 iteration)")
    ...
    scheduler.add_job(
        ...
        name='[TEST 1] Paper Trading 시작'
    )

    # TEST 2: 3분 30초 후 실행 (시작 + 3분 실행)
    scheduler.add_job(
        stop_paper_trading,
        DateTrigger(run_date=now + timedelta(seconds=210)),
        id='test_stop_trading',
        name='[TEST 2] Paper Trading 종료'
    )
