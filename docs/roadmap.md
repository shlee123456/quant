...
1. **모의투자 실행 파이프라인 없음**
   - `PaperTrader` 클래스는 있지만 실전 연결 안 됨
   - 대시보드 통합 미완성
...
- [ ] Paper Trading 탭 추가
- [ ] PaperTrader와 KIS 브로커 연결
- [ ] 여러 종목 동시 모의투자
- [ ] SQLite 데이터베이스 구현
- [ ] 전략 성과 비교 대시보드
- [ ] 단위 테스트 작성 (`tests/test_paper_trading_e2e.py`)
...
| Phase | 목표 | 기간 | 상태 |
|-------|------|------|------|
| **Phase 1** | 모의투자 엔드투엔드 연결 | 2-3주 | 🟡 진행 예정 |
| **Phase 2** | 자동화 스케줄러 | 2주 | ⚪ 대기 |
